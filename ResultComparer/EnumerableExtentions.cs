...
namespace ResultsComparer
{
    ...
    {
        public static IEnumerable<TResult> OuterZip<TResult, T1, T2>(this IEnumerable<T1> me, IEnumerable<T2> other,
                                                                     Func<T1, T2, int, TResult> resultor)
        {
            ...
            {
                foreach (var r in me)
                    yield return resultor(r, default(T2), i++);
                yield break;
                ;
            }
            ...
            using (var otherEnum = other.GetEnumerator())
                while (true)
                {
                    ...
                    yield return resultor(iHaveMore ? meEnum.Current : default(T1), heHasMore ? otherEnum.Current : default(T2), i++);
                }
        }
        ...
        {
            return me.OuterZip(other, (a, b, i) => resultor(a, b));
        }
            ...
            using (var otherEnum = other.GetEnumerator())
                while (true)
                {
                    if (meProgress)
                    {
                        ...
                        meAlign = iHaveMore ? meAligner(meEnum.Current, i) : default(TAlign);
                    }
                    if (otherProgress)
                    {
                        heHasMore = otherEnum.MoveNext();
                        otherAlign = heHasMore ? otherAligner(otherEnum.Current, i) : default(TAlign);
                    }
                    if (!(iHaveMore || heHasMore)) yield break;
                    if (!iHaveMore || !heHasMore || meAlign.Equals(otherAlign))
                    {
                        ...
                        otherProgress = true;
                        yield return
                            resultor(iHaveMore ? meEnum.Current : default(T1),
                                     heHasMore ? otherEnum.Current : default(T2), i++);
                    }
                    ...
                    {
                        otherProgress = false;
                        yield return resultor(meEnum.Current, default(T2), i++);
                    }
                    else if (otherAlign.Equals(prevOtherAlign))
                    {
                        meProgress = false;
                        yield return resultor(default(T1), otherEnum.Current, i++);
                    }
                    else
                    {
                        meProgress = true;
        ...
        {
            return me.OuterZip(other, (a, i) => meAligner(a), (b, i) => otherAligner(b), (a, b, i) => resultor(a, b));
        }
    }
}